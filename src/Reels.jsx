    ...
    <section className="reels-workspace">
      <aside className="panel reel-upload-card">
        ...
        <form onSubmit={submit}>
          ...
          <div className="video-publish-check"><span><b>Dashboard-only rollout</b><small>The reel will become ready for preview, but will not be published to the mobile feed yet.</small></span></div>
      ...
      <section className="panel reel-library-card">
        ...
        <div className="reel-library-list">
          {loading ? [...Array(4)].map((_, index) => <div className="reel-row-skeleton" key={index}/>) : reels.map((reel) => <article key={reel.id} className={`reel-library-row ${selectedId === reel.id ? 'selected' : ''}`} onClick={() => setSelectedId(reel.id)}>
            ...
            <div className="reel-row-controls"><label className={`reel-switch ${reel.status === 'published' ? 'on' : ''}`} title={reel.asset?.provider === 'cloudflare_stream' ? 'Mobile publishing will be enabled with the app playback migration' : reel.status === 'published' ? 'Disable reel' : 'Enable reel'}><input type="checkbox" checked={reel.status === 'published'} disabled={reel.asset?.provider === 'cloudflare_stream' || !['ready', 'published', 'disabled'].includes(reel.status)} onChange={() => changeStatus(reel)} onClick={(event) => event.stopPropagation()}/><i/></label><button onClick={(event) => { event.stopPropagation(); openPreview(reel); }} disabled={!['ready', 'published', 'disabled'].includes(reel.status)}>Preview</button></div>
          </article>)}
