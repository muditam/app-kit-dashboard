...
const BODY_AREAS = ['eyes', 'heart', 'liver', 'kidneys', 'nerves', 'feet'];
...
export default function QuizRouting({ onToast }) {
  ...
  const toggleBodyArea = (routeIndex, area) => setDraft((current) => ({
    ...current,
    routing: current.routing.map((route, index) => {
      if (index !== routeIndex) return route;
      const existing = route.bodyAreaSignals || [];
      const selected = existing.some((signal) => signal.area === area);
      return { ...route, bodyAreaSignals: selected ? existing.filter((signal) => signal.area !== area) : [...existing, { area, weight: 1 }] };
    }),
  }));
  const updateBodyAreaWeight = (routeIndex, area, weight) => setDraft((current) => ({
    ...current,
    routing: current.routing.map((route, index) => index === routeIndex ? {
      ...route,
      bodyAreaSignals: (route.bodyAreaSignals || []).map((signal) => signal.area === area ? { ...signal, weight } : signal),
    } : route),
  }));
  const updateOption = (index, value) => setDraft((current) => {
    const previous = current.options?.[index];
    return {
      ...current,
      options: (current.options || []).map((option, optionIndex) => optionIndex === index ? { label: value, value } : option),
      exclusiveOption: current.exclusiveOption === previous?.value ? value : current.exclusiveOption,
      routing: (current.routing || []).map((route) => route.answerValue === previous?.value ? { ...route, answerValue: value, answerLabel: value } : route),
    };
  });
  const updateCondition = (index, patch) => setDraft((current) => ({ ...current, conditions: (current.conditions || []).map((condition, conditionIndex) => conditionIndex === index ? { ...condition, ...patch } : condition) }));
  ...
  const save = async () => {
    ...
    try {
      const response = await api.updateQuizQuestion(draft._id, {
        ...
        options: draft.options, conditions: draft.conditions, routing: draft.routing, active: draft.active, order: draft.order, section: draft.section,
      });
      ...
      <main className="routing-editor panel">
        {!activeQuestion ? <div className="routing-empty"><MiniIcon name="spark" size={30}/><h3>Select a question</h3><p>Every question has a routing signal you can tune.</p></div> : <>
          ...
          <div className="routing-editor-body">
            <div className="routing-copy-card"><div className="routing-card-label"><span>QUESTION COPY</span><MiniIcon name="edit" size={14}/></div><input className="routing-question-input" value={activeQuestion.question} onChange={(event) => updateDraft({ question: event.target.value })}/><textarea className="routing-why-input" value={activeQuestion.why || ''} onChange={(event) => updateDraft({ why: event.target.value })} placeholder="Why this question matters..."/><input className="routing-question-input routing-placeholder-input" value={activeQuestion.placeholder || ''} onChange={(event) => updateDraft({ placeholder: event.target.value })} placeholder="Input placeholder (when applicable)"/><div className="routing-toggles"><label><input type="checkbox" checked={activeQuestion.required !== false} onChange={(event) => updateDraft({ required: event.target.checked })}/><span/>Required question</label><label><input type="checkbox" checked={Boolean(activeQuestion.autoAdvance)} onChange={(event) => updateDraft({ autoAdvance: event.target.checked })}/><span/>Auto-advance after selection</label></div></div>
            <div className="question-flow-grid">
              <div className="question-definition-card"><div className="routing-card-label"><span>ANSWER VALUES</span><small>{activeQuestion.options?.length || 0} choices</small></div>{activeQuestion.options?.length ? <div className="question-option-editor">{activeQuestion.options.map((option, index) => <label key={`${index}-${option.value}`}><span>{index + 1}</span><input value={option.label || ''} onChange={(event) => updateOption(index, event.target.value)}/></label>)}</div> : <p>This {activeQuestion.type.replace('_', ' ')} question uses its validation settings instead of fixed choices.</p>}{activeQuestion.exclusiveOption ? <label className="exclusive-value"><span>Exclusive answer</span><input value={activeQuestion.exclusiveOption} onChange={(event) => updateDraft({ exclusiveOption: event.target.value })}/></label> : null}</div>
              <div className="question-definition-card"><div className="routing-card-label"><span>DISPLAY CONDITIONS</span><small>{activeQuestion.conditions?.length || 0} rules</small></div>{activeQuestion.conditions?.length ? <div className="condition-editor">{activeQuestion.conditions.map((condition, index) => <div key={`${condition.field}-${index}`}><input value={condition.field || ''} onChange={(event) => updateCondition(index, { field: event.target.value })} placeholder="Answer field"/><select value={condition.operator || 'equals'} onChange={(event) => updateCondition(index, { operator: event.target.value })}><option value="equals">Equals</option></select><input value={condition.value ?? ''} onChange={(event) => updateCondition(index, { value: event.target.value })} placeholder="Required value"/></div>)}</div> : <p>Always shown when the member reaches this position in the quiz.</p>}<div className="validation-summary"><span>VALIDATION</span><code>{Object.keys(activeQuestion.validation || {}).length ? JSON.stringify(activeQuestion.validation) : 'No custom limits'}</code></div></div>
            </div>
            <div className="influence-header"><div><span className="routing-card-label">ANSWER INFLUENCE</span><h3>Where should each answer lead?</h3><p>Disease scores choose the Month 1 kit pathway. Body-area scores drive the six personalized report cards.</p></div><div className="live-preview"><span>TOP SIGNAL</span><strong>{topRoute?.answerLabel || '—'}</strong><small>{topRoute ? `${topRoute.weight} pts · ${topRoute.diseaseLabel || 'Unassigned'}` : 'No routing rule'}</small></div></div>
            <div className="route-table">
              <div className="route-table-head"><span>ANSWER</span><span>MATCH RULE</span><span>DISEASE PATHWAY</span><span>BODY FINDINGS</span><span>KIT SLUG(S)</span><span>WEIGHT</span></div>
              {options.map((route, index) => <div className="route-row" key={`${route.answerLabel}-${index}`}>
                <div className="answer-cell"><span className="answer-index">{index + 1}</span><input value={route.answerLabel || ''} onChange={(event) => updateRoute(index, { answerLabel: event.target.value, answerValue: event.target.value })}/></div>
                <div className="rule-cell"><select value={route.matchType || 'exact'} onChange={(event) => { const matchType = event.target.value; updateRoute(index, { matchType, sourceField: route.sourceField || (activeQuestion.key === 'body' ? 'bmi' : activeQuestion.key), answerValue: matchType === 'range' ? null : (route.answerValue || route.answerLabel) }); }}><option value="exact">Exact answer</option><option value="range">Numeric range</option></select>{route.matchType === 'range' ? <div className="range-inputs"><input type="number" value={route.minValue ?? ''} onChange={(event) => updateRoute(index, { minValue: event.target.value === '' ? null : Number(event.target.value) })} placeholder="Min"/><input type="number" value={route.maxValue ?? ''} onChange={(event) => updateRoute(index, { maxValue: event.target.value === '' ? null : Number(event.target.value) })} placeholder="Max"/><input value={route.unit || ''} onChange={(event) => updateRoute(index, { unit: event.target.value })} placeholder="Unit"/></div> : null}</div>
                <div className="pathway-cell"><select value={String(route.diseaseLabel || '').toLowerCase()} onChange={(event) => { const label = event.target.value; updateRoute(index, { diseaseLabel: title(label), diseaseKeys: label.split(' + ').map((item) => item.trim()) }); }}><option value="">Unassigned</option>{DISEASES.map((disease) => <option key={disease} value={disease}>{title(disease)}</option>)}</select><div className="pathway-chip"><i/> {route.diseaseLabel || 'Choose disease path'}</div></div>
                <div className="body-signal-cell">{BODY_AREAS.map((area) => { const signal = (route.bodyAreaSignals || []).find((item) => item.area === area); return <div className={`body-signal-chip ${signal ? 'selected' : ''}`} key={area}><button type="button" onClick={() => toggleBodyArea(index, area)}>{title(area)}</button>{signal ? <input aria-label={`${title(area)} weight`} type="number" min="0" max="10" value={signal.weight} onChange={(event) => updateBodyAreaWeight(index, area, Math.max(0, Math.min(10, Number(event.target.value) || 0)))}/> : null}</div>; })}{!route.bodyAreaSignals?.length && activeQuestion.key === 'hba1cKnown' ? <small className="branch-note">Selects the measurement branch only</small> : null}</div>
                <div className="kit-cell"><input value={(route.kitSlugs || []).join(', ')} onChange={(event) => updateRoute(index, { kitSlugs: event.target.value.split(',').map((item) => item.trim()).filter(Boolean) })} placeholder="diabetes-kit-1"/>{route.kitSlugs?.[0] ? <small>{kitsBySlug.get(route.kitSlugs[0])?.name || 'Custom / future kit'}</small> : null}</div>
                <div className="weight-cell"><input type="range" min="0" max="10" step="1" value={route.weight || 0} onChange={(event) => updateRoute(index, { weight: Number(event.target.value) })}/><strong>{route.weight || 0}</strong></div>
              </div>)}
            </div>
            <button className="add-route-button" onClick={() => setDraft((current) => ({ ...current, routing: [...(current.routing || []), { answerValue: 'New signal', answerLabel: 'New signal', diseaseKeys: ['diabetes'], diseaseLabel: 'Diabetes', kitSlugs: ['diabetes-kit-1'], weight: 1, bodyAreaSignals: [] }] }))}>+ Add answer signal</button>
          </div>
